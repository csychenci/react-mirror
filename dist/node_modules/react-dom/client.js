(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, (global.client = global.client || {}, global.client.js = factory(global.React, global.scheduler)));
	...
	class FiberRootNode {
	     ...
	     */
	    finishedWork;
	    pendingLanes; // 所有未被消费的 lane 的集合
	    finishedLane; // 本次更新消费的 lane
	    pendingPassiveEffects; // 需要执行的副作用
	...
	function initEvent(container, evnetType) {
	...
	function dispatchEvent(container, evnetType, e) {
	    ...
	    const { bubble, capture } = collectPaths(targetElement, container, evnetType);
	    // 2. 构造合成事件
	...
	function collectPaths(targetElement, container, evnetType) {
	    const paths = {
	        capture: [],
	        bubble: []
	    };
	    while (targetElement &&
	        ...
	        if (elementProps) {
	            const callbackNameList = getEventCallbackNameFromEventType(evnetType);
	            if (callbackNameList) {
	...
	function updateState() {
	    ...
	    const hook = updateWorkInProgressHook();
	    const queue = hook.updateQueue, pending = queue.shared.pending;
	...
	const commitMutationEffectsOnFiber = (finishedWork, root) => {
	    ...
	    if ((flags & Placement) !== NoFlags) {
	        ...
	        finishedWork.flags &= -2; // 从 flags 中移除 Placement
	    }
	    if ((flags & Update) !== NoFlags) {
	        commitUpdate(finishedWork);
	        finishedWork.flags &= -3; // 从 flags 中移除 Update
	    }
	    if ((flags & ChildDeletion) !== NoFlags) {
	        ...
	        finishedWork.flags &= -5; // 从 flags 中移除 Update
	    }
	...
	function commitPassiveEffects(fiber, root, type) {
	    ...
	    if (updateQueue !== null) {
	        if (updateQueue.lastEffect === null &&
	            true) {
	            console.error('当 FC 存在 PassiveEffect 时，不应该不存在 effect');
	        }
	...
	function insertOrAppendPlacementNodeIntoContainer(finishedWork, hostParent, before) {
	    if (finishedWork.tag === HostComponent ||
	        ...
	        if (before) {
	            insertChildToContainer(finishedWork.stateNode, hostParent, before.stateNode);
	        }
	        else {
	...
	function performSyncWorkOnRoot(root, updateLane) {
	...
	function commitRoot(root) {
	    ...
	    if ((finishedWork.flags & PassiveMask) !==
	        NoFlags ||
	    ...
	    const subtreeHasFlags = (finishedWork.subtreeFlags & MutationMask) !==
	        NoFlags;
	...
	function commitHookEffectList(flags, lastEffect, callack) {
